...
@section('mainEncabezado')
    Solicitudes Domicilio Lista
@endsection
@section('niveles')
    <li class="breadcrumb-item"><a class="text-dark" href="#">Principal</a></li>
    <li class="breadcrumb-item"><a class="text-dark" href="#">Solicitud</a></li>
                        ...
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table id="table" class="table m-0 table-bordered table-responsive">
                                    ...
                                    {{ csrf_field() }}
                                    <?php  $no = 1; ?>
                                    @foreach( $requesthomes as $requesthome )
                                        <tr class="cat{{$animal->id}}">

                                            <td>{{$requesthome->id}}</td>
                                            <td>{{$requesthome->date}}</td>
                                            <td>{{$requesthome->hour}}</td>
                                            <td>{{$requesthome->latitude}}</td>
                                            <td>{{$requesthome->longitude}}</td>
                                            <td>{{$requesthome->description}}</td>
                                            <td>{{$requesthome->created_at}}</td>
                                            <td>{{$requesthome->updated_at}}</td>
                                            <td width="10px">
                                                <a href="#" class="show-modal-cat btn btn-info btn-sm"
                                                   data-id-cat="{{$requesthome->id}}" data-name-cat="{{$requesthome->name}}"
                                                   data-body-cat="{{$requesthome->description}}">
                                                    <i class="fa fa-eye"></i>
                                                </a>
                                            </td>
                                            <td width="10px">
                                                <a href="#" class="edit-modal-cat btn btn-warning btn-sm"
                                                   data-id-cat="{{$requesthome->id}}"
                                                   data-name-cat="{{$requesthome->name}}" data-body-cat="{{$requesthome->description}}">
